...
|------|------|------|-------|----------|
| `STATE` | `{ IDLE: symbol; ROTATE: symbol; PAN: ...` | let/var | `{ IDLE: Symbol(), ROTATE: Symbol(), PAN: Symbol(), SCALE: Symbol(), FOV: Symb...` | ✗ |
| `INPUT` | `{ NONE: symbol; ONE_FINGER: symbol; O...` | let/var | `{ NONE: Symbol(), ONE_FINGER: Symbol(), ONE_FINGER_SWITCHED: Symbol(), TWO_FI...` | ✗ |
| `_center` | `{ x: number; y: number; }` | let/var | `{ x: 0, y: 0 }` | ✗ |
| `_transformation` | `{ camera: any; gizmos: any; }` | let/var | `{ camera: new Matrix4(), gizmos: new Matrix4() }` | ✗ |
...
| `_gizmoMatrixStateTemp` | `any` | let/var | `new Matrix4()` | ✗ |
| `_cameraMatrixStateTemp` | `any` | let/var | `new Matrix4()` | ✗ |
| `_scalePointTemp` | `any` | let/var | `new Vector3()` | ✗ |
| `_EPS` | `0.000001` | let/var | `0.000001` | ✗ |
| `restart` | `boolean` | let/var | `opState != this._state` | ✗ |
| `amount` | `number` | let/var | `Math.max( distance / this._tbRadius, angle ) * this.rotateSpeed` | ✗ |
...
| `deltaTime` | `number` | let/var | `( performance.now() - this._timeCurrent )` | ✗ |
| `self` | `this` | let/var | `this` | ✗ |
| `self` | `this` | let/var | `this` | ✗ |
| `rotationPoint` | `any` | let/var | `*not shown*` | ✗ |
| `amount` | `number` | let/var | `MathUtils.DEG2RAD * ( this._startFingerRotation - this._currentFingerRotation )` | ✗ |
| `minDistance` | `12` | let/var | `12` | ✗ |
| `amount` | `number` | let/var | `this._currentFingerDistance / this._startFingerDistance` | ✗ |
...
| `gizmoX` | `any` | let/var | `this._gizmos.children[ 0 ]` | ✗ |
...
| `h` | `any` | let/var | `this._v3_1.z` | ✗ |
| `cameraGizmoDistance` | `any` | let/var | `*not shown*` | ✗ |
| `m` | `number` | let/var | `h / l` | ✗ |
| `q` | `any` | let/var | `cameraGizmoDistance` | ✗ |
...
### `ArcballControls.connect(element: any): void`
...
### `ArcballControls.onSinglePanMove(event: any, opState: any): void`
...
### `ArcballControls.onRotateMove(): void`
...
### `ArcballControls.getRaycaster(): Raycaster`
...
### `ArcballControls.unprojectOnObj(cursor: Vector2, camera: Camera): Vector3`
...
### `ArcballControls.unprojectOnTbSurface(camera: Camera, cursorX: number, cursorY: number, canvas: HTMLElement, tbRadius: number): Vector3`
...
- `Math.sqrt`
- `this.getCursorNDC`
- `this._v3_1.applyMatrix4`
- `this._v3_1.clone().normalize`
- `camera.position.distanceTo`
- `rayDir.set`
- `this._v2_1.setX`
- `this._v2_1.setY`
- `this._v2_1.angle`
- `rayDir.multiplyScalar`

**Internal Comments:**
```
//intersection with sphere (x10)
//intersection with hyperboloid (x8)
//unproject cursor on the near plane (x5)
//	  camera (x2)
//		|\ (x2)
//		| \ (x2)
//		|  \ (x2)
...
//		| 	 \ (x2)
//		| 	  \ (x2)
//	_ _ | _ _ _\ _ _  near plane (x2)
//			l (x2)
//ray aligned with camera (x4)
/*
			 ...
			 */ (x2)
//if angle between intersection point and X' axis is >= 45°, return that point (x2)
//otherwise, calculate intersection point with hyperboloid (x2)
/*
			 *|y = m * x + q
			 *|y = (1 / x) * (r^2 / 2)
			 *
...
### `ArcballControls.unprojectOnTbPlane(camera: Camera, cursorX: number, cursorY: number, canvas: HTMLElement, initialDistance: boolean): Vector3`
...
- `Math.sqrt`
- `Math.pow`
- `this._v3_1.setFromMatrixPosition( this._cameraMatrixState0 ).distanceTo`
- `this._v3_2.setFromMatrixPosition`
- `camera.position.distanceTo`
- `rayDir.set`
- `rayDir.multiplyScalar`
...
### `ArcballControls.updateMatrixState(): void`
...
**Internal Comments:**
```
//update camera and gizmos state (x5)
```
...
### `ArcballControls.updateTbState(newState: { IDLE: symbol; ROTATE: symbol; PAN: symbol; SCALE: symbol; FOV: symbol; FOCUS: symbol; ZROTATE: symbol; TOUCH_MULTI: symbol; ANIMATION_FOCUS: symbol; ANIMATION_ROTATE: symbol; }, updateMatrices: boolean): void`
...
- **`newState`** `{ IDLE: symbol; ROTATE: symbol; PAN: symbol; SCALE: symbol; FOV: symbol; FOCUS: symbol; ZROTATE: symbol; TOUCH_MULTI: symbol; ANIMATION_FOCUS: symbol; ANIMATION_ROTATE: symbol; }`
...
- `this.target.equals`
- `this._gizmos.position.copy`
- `this.calculateTbRadius`
- `this.makeGizmos`
- `this._currentTarget.copy`
- `MathUtils.clamp`
- `this.applyTransformMatrix`
- `this.scale`
- `this.object.position.distanceTo`
...
### `ArcballControls.setStateFromJSON(json: any): void`
...
**Internal Comments:**
```
//prevent only if button 2 is actually used (x4)
```
...
- `this._downEvents.push`
- `performance.now`
- `this._touchStart.push`
- `this._touchCurrent.push`
...
- `this.getOpStateFromAction`

**Internal Comments:**
```
//singleMove (x8)
//rotate/pan/pinchMove (x4)
//multMove (x4)
//checkDistance
```
...
//singleEnd (x4)
//doubleEnd (x4)
//switching to singleStart (x4)
//multCancel (x4)
//first valid click detected (x4)
//second valid click detected (x4)
//fire double tap and reset values (x4)
//new 'first click' (x4)
```
...
- `this.updateTbState`
- `Math.pow`
- `this.unprojectOnTbPlane( this.object, event.clientX, event.clientY, this.domElement ).applyQuaternion( this.object.quaternion ).multiplyScalar( 1 / this.object.zoom ).add`
- `this.unprojectOnTbPlane( this.object, event.clientX, event.clientY, this.domElement ).applyQuaternion( this.object.quaternion ).add`
- `this.applyTransformMatrix`
- `this.scale`
- `this.disposeGrid`
- `this.drawGrid`
...
##### `compareMouseAction(action1: any, action2: any): boolean`
...
##### `getOpFromAction(mouse: 0 | 1 | 2 | "WHEEL", key: "CTRL" | "SHIFT"): "PAN" | "ROTATE" | "ZOOM" | "FOV"`
...
##### `getOpStateFromAction(mouse: 0 | 1 | 2, key: "CTRL" | "SHIFT"): { IDLE: symbol; ROTATE: symbol; PAN: symbol; SCALE: symbol; FOV: symbol; FOCUS: symbol; ZROTATE: symbol; TOUCH_MULTI: symbol; ANIMATION_FOCUS: symbol; ANIMATION_ROTATE: symbol; }`
...
##### `getAngle(p1: PointerEvent, p2: PointerEvent): number`
...
##### `updateTouchEvent(event: PointerEvent): void`
...
##### `applyTransformMatrix(transformation: any): void`
...
##### `calculateAngularSpeed(p0: number, p1: number, t0: number, t1: number): number`
...
##### `calculatePointersDistance(p0: PointerEvent, p1: PointerEvent): number`
...
##### `calculateTbRadius(camera: Camera): number`
...
##### `getCursorNDC(cursorX: number, cursorY: number, canvas: HTMLElement): Vector2`
...
##### `getCursorPosition(cursorX: number, cursorY: number, canvas: HTMLElement): Vector2`
...
##### `onFocusAnim(time: number, point: Vector3, cameraMatrix: Matrix4, gizmoMatrix: Matrix4): void`
...
##### `onRotationAnim(time: number, rotationAxis: Vector3, w0: number): void`
...
##### `pan(p0: Vector3, p1: Vector3, adjust: boolean): any`
...
##### `rotate(axis: Vector3, angle: number): any`
...
##### `scale(size: number, point: Vector3, scaleGizmos: boolean): any`
...
##### `setTransformationMatrices(camera: Matrix4, gizmos: Matrix4): void`
...
##### `zRotate(point: Vector3, angle: number): any`
...
##### `unprojectOnObj(cursor: Vector2, camera: Camera): Vector3`
...
##### `unprojectOnTbSurface(camera: Camera, cursorX: number, cursorY: number, canvas: HTMLElement, tbRadius: number): Vector3`
...
##### `unprojectOnTbPlane(camera: Camera, cursorX: number, cursorY: number, canvas: HTMLElement, initialDistance: boolean): Vector3`
...
##### `updateTbState(newState: { IDLE: symbol; ROTATE: symbol; PAN: symbol; SCALE: symbol; FOV: symbol; FOCUS: symbol; ZROTATE: symbol; TOUCH_MULTI: symbol; ANIMATION_FOCUS: symbol; ANIMATION_ROTATE: symbol; }, updateMatrices: boolean): void`

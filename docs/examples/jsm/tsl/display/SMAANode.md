...
| `areaTextureImage` | `HTMLImageElement` | let/var | `new Image()` | ✗ |
| `searchTextureImage` | `HTMLImageElement` | let/var | `new Image()` | ✗ |
| `SMAA_THRESHOLD` | `0.1` | let/var | `0.1` | ✗ |
| `SMAA_MAX_SEARCH_STEPS` | `8` | let/var | `8` | ✗ |
| `SMAA_AREATEX_MAX_DISTANCE` | `16` | let/var | `16` | ✗ |
| `SMAA_AREATEX_SUBTEX_SIZE` | `number` | let/var | `( 1 / 7 )` | ✗ |
...
### `SMAANode.getTextureNode(): PassTextureNode`
...
### `SMAANode.setSize(width: number, height: number): void`
...
- `this._renderTargetEdges.setSize`
- `this._renderTargetWeights.setSize`
- `this._renderTargetBlend.setSize`
...
### `SMAANode.updateBefore(frame: NodeFrame): void`
...
### `SMAANode.setup(builder: NodeBuilder): PassTextureNode`
...
- `vec4( this._invSize.xy, this._invSize.xy ).mul`
- `vec4 (from three/tsl)`
- `varyingProperty( 'vec4', 'vOffset0' ).assign`
- `varyingProperty( 'vec4', 'vOffset1' ).assign`
- `varyingProperty( 'vec4', 'vOffset2' ).assign`
- `varyingProperty (from three/tsl)`
- `vec4().toVar`
- `this.textureNode.sample( uvNode ).rgb.toVar`
- `this.textureNode.sample( vOffset0.xy ).rgb.toVar`
- `abs (from three/tsl)`
- `C.sub`
- `max (from three/tsl)`
- `this.textureNode.sample( vOffset0.zw ).rgb.toVar`
- `step( threshold, delta.xy ).toVar`
- `dot( edges, vec2( 1.0, 1.0 ) ).equal( 0 ).discard`
- `this.textureNode.sample( vOffset1.xy ).rgb.toVar`
- `this.textureNode.sample( vOffset1.zw ).rgb.toVar`
- `max( delta.x, delta.y, delta.z, delta.w ).toVar`
- `this.textureNode.sample( vOffset2.xy ).rgb.toVar`
- `this.textureNode.sample( vOffset2.zw ).rgb.toVar`
- `edges.xy.mulAssign`
- `step (from three/tsl)`
- `float( 0.5 ).mul`
- `vec2( e ).toVar`
...
- `float( 255 ).mul`
- `searchTex.sample`
- `float( SMAA_AREATEX_MAX_DISTANCE ).mul( float( 4 ).mul( vec2( e1, e2 ) ).round() ).add`
- `SMAA_AREATEX_PIXEL_SIZE.mul( texcoord ).add`
- `texcoord.y.addAssign`
- `float( SMAA_AREATEX_SUBTEX_SIZE ).mul`
- `areaTex.sample`
- `vec2( 0.0, 1.0 ).toVar`
- `vec2( texcoord ).toVar`
- `Loop (from three/tsl)`
...
- `vec2( 2, 0 ).mul`
- `If (from three/tsl)`
- `coord.x.lessThanEqual( end ).or`
- `e.g.lessThanEqual( float( 0.8281 ) ).or`
- `e.r.notEqual`
...
- `SMAASearchLength`
- `coord.addAssign`
- `coord.x.greaterThanEqual( end ).or`
- `vec2( 1.0, 0.0 ).toVar`
- `vec2( 0, - 2 ).mul`
- `coord.y.lessThanEqual( end ).or`
- `e.r.lessThanEqual( float( 0.8281 ) ).or`
- `e.g.notEqual`
- `coord.y.addAssign`
- `coord.y.subAssign`
- `this._invSize.y.mul`
- `coord.y.greaterThanEqual( end ).or`
- `uvNode.xy.div`
- `vec4( uvNode.xy, uvNode.xy ).add( vec4( this._invSize.xy, this._invSize.xy ).mul( vec4( - 0.25, - 0.125, 1.25, - 0.125 ) ) ).toVar`
- `vec4( uvNode.xy, uvNode.xy ).add( vec4( this._invSize.xy, this._invSize.xy ).mul( vec4( - 0.125, - 0.25, - 0.125, 1.25 ) ) ).toVar`
- `vec4( vOffset0.xz, vOffset1.yw ).add( vec4( - 2.0, 2.0, - 2.0, 2.0 ).mul( vec4( this._invSize.xx, this._invSize.yy ) ).mul( float( SMAA_MAX_SEARCH_STEPS ) ) ).toVar`
- `varyingProperty( 'vec2', 'vPixcoord' ).assign`
- `vec4( 0.0, 0.0, 0.0, 0.0 ).toVar`
- `this._edgesTextureUniform.sample( uvNode ).rg.toVar`
- `e.g.greaterThan`
- `vec2().toVar`
- `SMAASearchXLeft`
- `this._edgesTextureUniform.sample( coordsLeft ).r.toVar`
- `SMAASearchXRight`
- `d.div( this._invSize.x ).sub`
- `sqrt (from three/tsl)`
- `this._edgesTextureUniform.sample( coordsRight.add( vec2( 1, 0 ).mul( this._invSize ) ) ).r.toVar`
- `SMAAArea`
- `e.r.greaterThan`
- `SMAASearchYUp`
- `this._edgesTextureUniform.sample( coordsUp ).g.toVar`
- `SMAASearchYDown`
- `d.div( this._invSize.y ).sub`
- `this._edgesTextureUniform.sample( coordsDown.add( vec2( 0, 1 ).mul( this._invSize ) ) ).g.toVar`
- `this._weightsTextureUniform.sample`
- `If( dot( a, vec4( 1.0 ) ).lessThan( 1e-5 ), () => { // Edge at north

				result.assign( this.textureNode.sample( uvNode ) );

			} ).Else`
- `a.a.greaterThan( a.b ).select`
- `a.b.negate`
- `a.g.greaterThan( a.r ).select`
- `a.r.negate`
...
- `offset.x.assign`
- `this.textureNode.sample( uvNode ).toVar`
- `vec2( uvNode ).toVar`
- `texcoord.addAssign`
- `sign( offset ).mul`
- `this.textureNode.sample( texcoord ).toVar`
- `abs( offset.x ).greaterThan( abs( offset.y ) ).select( abs( offset.x ), abs( offset.y ) ).toVar`
- `mix (from three/tsl)`
- `result.assign`
- `SMAAEdgeDetectionVS().context`
- `builder.getSharedContext`
- `SMAAEdgeDetectionFS().context`
- `SMAAWeightsVS().context`
- `SMAAWeightsFS().context`
- `SMAABlendVS().context`
- `SMAABlendFS().context`
...
```
// edges (x2)
// Calculate color deltas: (x2)
// Calculate left and top deltas: (x2)
// We do the usual threshold: (x2)
// Then discard if there is no edge: (x7)
// Calculate right and bottom deltas: (x2)
// Calculate the maximum delta in the direct neighborhood: (x2)
// Calculate left-left and top-top deltas: (x2)
// Calculate the final maximum delta: (x3)
// Local contrast adaptation in action: (x5)
// weights (x2)
// Not required if searchTex accesses are set to point: (x2)
// float2 SEARCH_TEX_PIXEL_SIZE = 1.0 / float2(66.0, 33.0); (x2)
// e = float2(bias, 0.0) + 0.5 * SEARCH_TEX_PIXEL_SIZE + e * float2(scale, 1.0) * float2(64.0, 32.0) * SEARCH_TEX_PIXEL_SIZE; (x2)
// Rounding prevents precision errors of bilinear filtering: (x2)
// We do a scale and bias for mapping to texel space: (x3)
// Move to proper place, according to the subpixel offset: (x5)
/**
			* @PSEUDO_GATHER4
			...
			*/ (x2)
// We correct the previous (-0.25, -0.125) offset we applied: (x5)
// The searches are bias by 1, so adjust the coords accordingly: (x5)
// Disambiguate the length added by the last step: (x5)
// We will use these offsets for the searches later on (see @PSEUDO_GATHER4): (x2)
// And these for the searches, they indicate the ends of the loops: (x2)
// Find the distance to the left: (x2)
// Now fetch the left crossing edges, two at a time using bilinear (x2)
// filtering. Sampling at -0.25 (see @CROSSING_OFFSET) enables to (x2)
// discern what value each edge has: (x2)
// Find the distance to the right: (x2)
// We want the distances to be in pixel units (doing this here allow to (x3)
// better interleave arithmetic and memory accesses): (x3)
// SMAAArea below needs a sqrt, as the areas texture is compressed quadratically: (x4)
// Fetch the right crossing edges: (x2)
// Get the area for this direction: (x8)
// Find the distance to the top: (x2)
// Fetch the top crossing edges: (x2)
// Find the distance to the bottom: (x2)
// We want the distances to be in pixel units: (x3)
// Fetch the bottom crossing edges: (x2)
// blend (x2)
//const vOffset0 = vec4( uvNode.xy, uvNode.xy ).add( vec4( this._invSize.xy, this._invSize.xy ).mul( vec4( - 1.0, 0.0, 0.0, - 1.0 ) ) ); (x2)
//varyingProperty( 'vec4', 'vOffset0' ).assign( vOffset0 ); (x5)
//const vOffset0 = varyingProperty( 'vec4', 'vOffset0' ); (x2)
// Fetch the blending weights for current pixel: (x2)
// Is there any blending weight with a value greater than 0.0? (x5)
// Up to 4 lines can be crossing a pixel (one through each edge). We (x2)
// favor blending by choosing the line with the maximum weight for each (x2)
// direction: (x2)
// Then we go in the direction that has the maximum weight: (x5)
// Fetch the opposite color and lerp by hand: (x2)
```
...
### `SMAANode._getAreaTexture(): string`
...
### `SMAANode._getSearchTexture(): string`
...
```typescript
( node ) => nodeObject( new SMAANode( convertToTexture( node ) ) )
```
</details>
